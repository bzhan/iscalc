...
class BinomTest(unittest.TestCase):
    ...
    def testExample01(self):
        ...
        goal05 = file.add_goal("(x+y)^n = SUM(k,0,n,C(n, k)*x^k*y^(n-k))", conds=['x!=0', 'y!=0', 'n>0'], fixes=fixes)
        ...
        s1 = parser.parse_expr("x * SUM(k, 0, n, x ^ k * y ^ (-k + n) * C(n,k))", fixes=fixes)
        s2 = parser.parse_expr("SUM(k, 0, n, x * (x ^ k * y ^ (-k + n) * C(n,k)))", fixes=fixes)
        calc.perform_rule(rules.Equation(s1, s2))
        s1 = parser.parse_expr("y * SUM(k, 0, n, x ^ k * y ^ (-k + n) * C(n,k))", fixes=fixes)
        s2 = parser.parse_expr("SUM(k, 0, n, y * (x ^ k * y ^ (-k + n) * C(n,k)))", fixes=fixes)
        calc.perform_rule(rules.Equation(s1, s2))
        s1 = parser.parse_expr("x * (x ^ k * y ^ (-k + n) * C(n,k))", fixes=calc.ctx.get_fixes())
        s2 = parser.parse_expr("C(n,k)*x^(k+1)*y^(n-k)", fixes=calc.ctx.get_fixes())
        calc.perform_rule(rules.Equation(s1, s2))
        s1 = parser.parse_expr("y * (x ^ k * y ^ (-k + n) * C(n,k))", fixes=calc.ctx.get_fixes())
        s2 = parser.parse_expr("C(n,k)*x^k*y^(n-k+1)", fixes=calc.ctx.get_fixes())
        calc.perform_rule(rules.Equation(s1, s2))
        ...
        calc.perform_rule(rules.FullSimplify())
        calc.perform_rule(rules.OnLocation(rules.ExpandDefinition('C'), '0.1.1'))
        calc.perform_rule(rules.OnLocation(rules.ExpandDefinition('C'), '1.1'))
        calc.perform_rule(rules.OnLocation(rules.ChangeSummationIndex("1"), '0.0.0'))
        calc.perform_rule(rules.FullSimplify())
        s1 = parser.parse_expr("SUM(i, 1, n, x ^ i * y ^ (-i + n + 1) * C(n,i - 1)) + SUM(i, 1, n, x ^ i * y ^ (-i + n + 1) * C(n,i))", fixes=calc.ctx.get_fixes())
        s2 = parser.parse_expr("SUM(i, 1, n, x ^ i * y ^ (-i + n + 1) * C(n,i - 1) + x ^ i * y ^ (-i + n + 1) * C(n,i))", fixes=calc.ctx.get_fixes())
        calc.perform_rule(rules.Equation(s1, s2))
        s1 = parser.parse_expr(
            "x ^ i * y ^ (-i + n + 1) * C(n,i - 1) + x ^ i * y ^ (-i + n + 1) * C(n,i)",
            fixes=calc.ctx.get_fixes())
        s2 = parser.parse_expr(
            "x ^ i * y ^ (-i + n + 1) * (C(n,i - 1) + C(n,i))",
            fixes=calc.ctx.get_fixes())
        calc.perform_rule(rules.Equation(s1, s2))
        ...
        calc.perform_rule(rules.OnLocation(rules.SplitSummation(cond), '1'))
        calc.perform_rule(rules.OnLocation(rules.ExpandDefinition('C'), '0.0.1'))
        calc.perform_rule(rules.OnLocation(rules.ExpandDefinition('C'), '1.1.0.1'))
        calc.perform_rule(rules.FullSimplify())
